...
* `format(n, format, local:, thousandsSeparator:, decimalSeparator:., thousandthsSeparator:)`: Formats the number according to the specified format string (uses Lua `string.format`). `local` can be `empty` (1055.2 -> 1055.2), `en` or `us` (1055.2 -> 1,055.2) or `fr` (1055.2 -> 1 055,2). `local` can also be set to `custom`, and `thousandsSeparator`, etc... will be used. This macro will be automatically called for any concatenation of a string and a number. Options used can be customised using the contextual variables `local`, `localNumberFormat`, `localThousandsSeparator`, `localDecimalSeparator`, and `localThousandthsSeparator`.
